...
from crystalgfh.common.data_utils import lattice_params_to_matrix_torch, get_pbc_distances, radius_graph_pbc, frac_to_cart_coords, repeat_blocks
from crystalgfh.pl_modules.attention import CrossAttention
...
class CSPLayer(nn.Module):
    ...
    def __init__(
        ...
        self.cross_attn = CrossAttention(hidden_dim, hidden_dim, hidden_dim, hidden_dim, num_heads)
    ...
    def node_model(self, node_features, edge_features, edge_index):
        ...
        out = self.cross_attn(node_features, agg)
        return out
...
class CSPNet(nn.Module):
    ...
    def __init__(
        ...
        # self.prop_out = nn.Linear(hidden_dim_before_out, 1, bias = False)
    ...
    def forward(self, t, c, atom_types, frac_coords, lattices, num_atoms, node2graph):
        ...
        condition_features = c.repeat_interleave(num_atoms, dim=0)
        t_per_atom = t.repeat_interleave(num_atoms, dim=0)
        # print('node_features_shape: ', node_features.shape)
        # print('condition_features_shape: ', condition_features.shape)
        # print('t_per_atom_shape: ', t_per_atom.shape)
        node_features = torch.cat([node_features, condition_features, t_per_atom], dim=1)
        ...
        # prop_out = self.prop_out(graph_features)
        
        if self.pred_type:
            type_out = self.type_out(node_features)
            return lattice_out, coord_out, type_out
        # return lattice_out, coord_out, prop_out
