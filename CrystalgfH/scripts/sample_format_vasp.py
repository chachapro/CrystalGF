...
import torch
from pymatgen.io.cif import CifWriter
from sample_api import construct_dataset_from_vasp_format, construct_dataset_from_syminfo, generate_structures_from_dataset, diffusion, get_pymatgen
import os
import pandas as pd
...
def main(args):
    ...
    if args.json_file != '':
        dataset = construct_dataset_from_vasp_format(args.json_file)
    ...
    for data in dataset:
        ...
        form_list.append(str(data['formation_energy_per_atom']))
    ...
    for i,structure in enumerate(structure_list):
        tar_file = os.path.join(tar_dir, f"%05d_{formula_list[i]}_{spg_list[i]}_{form_list[i]}.cif"%i)
    ...
    with open("./llama_full_form_2diff.json", 'w', encoding='utf-8') as f:
